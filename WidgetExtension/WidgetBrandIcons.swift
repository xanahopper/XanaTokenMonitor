...
/// 小组件扩展内置的品牌图标(与主程序 BrandIcons.swift 同源)
enum WidgetBrandIcon {
    ...
    static func image(named assetName: String) -> Image? {
        let data: Data?
        switch assetName {
        case "brand-openai": data = openai
        case "brand-anthropic": data = anthropic
        case "brand-kimi": data = kimi
        case "brand-xiaomi": data = xiaomi
        case "brand-zhipu": data = zhipu
        default: data = nil
        }
